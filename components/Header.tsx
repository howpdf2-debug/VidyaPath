  ...
  return (
    <header className="sticky top-0 z-50 bg-white/90 backdrop-blur-md shadow-sm border-b border-orange-100">
      <nav className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between gap-3">
        ...
        {/* Desktop Nav Links */}
        <div className="hidden lg:flex gap-1 bg-gray-50/50 p-1 rounded-full overflow-x-auto">
          {navLinks.map(link => (
            <Link
              key={link.href}
              href={link.href}
              className="flex items-center gap-1.5 px-4 py-2 text-sm font-medium rounded-full text-gray-700 hover:bg-gradient-to-r hover:from-orange-100 hover:to-pink-100 transition whitespace-nowrap"
            >
              <span>{link.icon}</span>
        ...
        <button
          ...
          className="md:hidden p-2 rounded-full bg-orange-50"
